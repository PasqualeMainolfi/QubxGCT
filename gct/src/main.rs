    ...
    match MODE {
        GranulatorMode::Sound => {
            ...
            loop {
                let grain = Grain::new(
                    grain_params.dur,
                    grain_params.amp,
                    grain_params.pan_pos,
                    grain_params.env_mode.clone(),
                );
                let g = grain.generate_sound_grain(
                    audio_sigs[0],
                    grain_params.time_pos,
                    grain_params.freq,
                    grain_params.rev,
                );
        ...
        GranulatorMode::Microphone => {
            ...
            let dsp_function = {
                ...
                let mut grain_length: usize =
                    (((gparams.dur as f32 / CHUNK as f32).ceil()) * CHUNK as f32) as usize;
                let mut env = Envelope::generate_envelope(EnvelopeMode::Hanning, grain_length);
                let mut tot_delay_frame_silence =
                    (gparams.delay * SR as f32 / CHUNK as f32).ceil() as i32;
                ...
                move |frame: &[f32]| {
                    ...
                    match hop.cmp(&grain_length) {
                        std::cmp::Ordering::Less => {
                            let grain =
                                Grain::new(frame.len() as i32, gparams.amp, gparams.pan_pos, None);
                            let mut g = grain.generate_sound_grain(
                                frame,
                                gparams.time_pos,
                                gparams.freq,
                                gparams.rev,
                        ...
                        std::cmp::Ordering::Equal | std::cmp::Ordering::Greater => {
                            if delay_frame_silence_count < tot_delay_frame_silence {
                                ...
                                grain_length = (((gparams.dur as f32 / CHUNK as f32).ceil())
                                    * CHUNK as f32)
                                    as usize;
                                env = Envelope::generate_envelope(
                                    EnvelopeMode::Hanning,
                                    grain_length,
                                );
                                tot_delay_frame_silence =
                                    (gparams.delay * SR as f32 / CHUNK as f32).ceil() as i32;
                            };
